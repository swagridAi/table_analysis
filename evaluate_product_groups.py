#!/usr/bin/env python3
"""
Script to evaluate potential product groups using co-occurrence metrics.
"""
...
def main():
    ...
    strategies = [
        ("Community-based", community_metrics["overall_metrics"]["quality_score"] if 'community_metrics' in locals() else 0),
        ("Table-based", table_metrics["overall_metrics"]["quality_score"]),
        ("Custom", custom_metrics["overall_metrics"]["quality_score"])
    ]
    
    for name, score in sorted(strategies, key=lambda x: x[1], reverse=True):
        print(f"{name}: Quality Score = {score:.4f}")
    
    print("\nAnalysis complete! Results saved to:", metrics_dir)
